...
@implementation BWaterfallLayout
...
- (void)prepareLayout{
    ...
    float top = 0;
    for (int i = 0; i < CollectionViewColCount; i++) {
...
- (void)layoutItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSLog(@"top:%f",top);
    //确定cell的frame
    CGRect frame = CGRectMake(edgeInsets.left + col * (edgeInsets.left + itemSize.width+edgeInsets.right), top + edgeInsets.top+edgeInsets.bottom, itemSize.width, itemSize.height);
    NSLog(@"frme:%@",NSStringFromCGRect(frame));
    //更新列高
    [_colArr replaceObjectAtIndex:col withObject:[NSNumber numberWithFloat:top + edgeInsets.top+edgeInsets.bottom + itemSize.height]];
